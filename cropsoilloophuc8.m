...
basemap   = 'HUC8_30m.nc'; % the spatial units
...
outfile   = 'huc8soilcdlloop.csv';  %output filename

basevar       = 'huc8'; % the spatial variable name in basemap
...
huc8list = readtable('huc8list.csv');

f = waitbar(0,'1','Name','Tabulation via looping',...
...
for ci = 1:nobs
    ...
    waitbar(ci/nobs,f,sprintf('Working on %d th HUC8',ci)); 

    fprintf('Working on the %d th huc8 of %d\n',ci,units(ci));
    ...
    eval(strcat('tmp = basetable(basetable.',basevar,'==units(',num2str(ci),'),:);'));
    ...
    for yi = 1:length(years)
        ...
        temptable = array2table(B,'VariableNames',{'huc8id','soil','crop','area'});
        temptable.year = year*ones(size(B,1),1);
        temptable.huc8 = ones(size(B,1),1)*huc8list.HU8(huc8list.huc8==units(ci));
